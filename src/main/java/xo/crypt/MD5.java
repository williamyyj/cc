...
// $Id: MD5.java,v 1.1 2005/09/13 14:14:50 administrator Exp $
//
...
public class MD5  {
	...
	*/
	...
	public String encode(byte[] b){
		...
		update(b);
		md5final(out);
		return  Hex.toHex(out);
	}
	
	public String encode(String fname) {
		try {
			return encode(new FileInputStream(fname));
	...
	public String encode(File f) {
		try {
			return encode(new FileInputStream(f));
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		}
		return null;
	...
	public String encode(InputStream is){
		int len = 0 ;
